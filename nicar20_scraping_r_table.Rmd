...
Not going to run it for time's sake, but after running it you will realize the first column is always empty. And since the second column is also useless, so we can delete the first two columns in the function.
...
readAct_Num <- function(act_num) {
  ...
  table <- table[-1,-c(1:2)] ## deletes the first column now
